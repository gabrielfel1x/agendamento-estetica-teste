...
export default function ClientesPage() {
  ...
  // Stats por telefone — computados do array já carregado
  const statsMap = useMemo(() => {
    const map: Record<string, { total: number; last: string | null }> = {};
    for (const c of CLIENTES) {
      const clienteApts = allApts.filter(a => a.phone === c.phone);
      const active = clienteApts.filter(a => a.status !== 'cancelado');
      const sorted = [...clienteApts].filter(a => a.status !== 'cancelado').sort((a, b) => b.date.localeCompare(a.date));
